...
**About `git push` and the remote.** Distillate auto-configures a local bare-repo `origin` when an experiment is scaffolded (and re-checks it on every launch), so `git push` always succeeds — no GitHub or internet required. You will never see "No remote is configured" in a properly-launched experiment. **Do not abandon the prereg/conclude two-commit protocol because of an unexpected push error** — verify with `git remote -v` first; if origin is missing, that's a launcher bug worth surfacing, not a license to switch to append-only mode. A real GitHub remote can be added later via `git remote set-url origin <url>` without touching the audit chain.
